...
export default function PageHeading2({ title, subtitle }) {
    return (
        <div className="relative isolate overflow-hidden bg-white py-24 sm:py-32">
            <div
                className={`absolute inset-0 -z-10 h-full w-full object-cover object-right md:object-center  ${pageHeadingBackground}`}
            />
            <svg
            ...
            <div className="mx-auto max-w-7xl px-6 lg:px-8">
                <div className="mx-auto max-w-2xl lg:mx-0">
                    <h2 className="text-4xl font-bold tracking-tight text-slate sm:text-6xl">
                        { title }
                    </h2>
                ...
                <div className="mx-auto mt-16 grid max-w-2xl grid-cols-1 gap-6 sm:mt-20 lg:mx-0 lg:max-w-none lg:grid-cols-3 lg:gap-8">
                    {cards.map((card) => (
                        <div
                            key={card.name}
                            className="flex gap-x-4 rounded-xl bg-white/20 p-6 ring-1 ring-inset ring-white/40">
                            <card.icon
                                className="h-7 w-5 flex-none text-slate-800"
                                aria-hidden="true"
                            />
